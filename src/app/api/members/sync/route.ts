...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const syncLogResult = await query(
      ...
      ['Member', 'incremental', 'running', new Date()]
    );
    ...
    for (const mlsMember of members) {
      try {
        const existingMember = await memberService.getMemberByKey(
          mlsMember.MemberKey
        );

        if (mlsMember.MlgCanView) {
          await memberService.upsertMember(mlsMember);
          ...
          if (existingMember) {
            await memberService.deleteMember(mlsMember.MemberKey);
            recordsDeleted++;
          }
        }
      } catch (error) {
        console.error(`Error processing member ${mlsMember.MemberKey}:`, error);
      }
    }
